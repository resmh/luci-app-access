...
var cbiACLSelect = form.Value.extend({
	renderWidget: function(section_id) {
			...
			E('tr', { 'class': 'tr' }, [
				E('td', { 'class': 'td' }, [
					...
					E('a', { 'href': '#', 'click': function() {
						table.querySelectorAll('tr:nth-child(n+3)').forEach(function(el) { el.style.display = 'none'; });
					} }, [ _('none', 'Hide ACLs') ]), ' | ',
					E('a', { 'href': '#', 'click': function() {
						table.querySelectorAll('tr:nth-child(n+3)').forEach(function(el) { el.style.display = (el.childNodes[2].childNodes[0].value) ? '' : 'none'; });
					} }, [ _('selected', 'Show only selected ACLs') ]), ' | ',
					E('a', { 'href': '#', 'click': function() {
						table.querySelectorAll('tr:nth-child(n+3)').forEach(function(el) { el.style.display = ''; });
					} }, [ _('all', 'Show all ACLs') ])
				]),
				E('td', { 'class': 'td' }, [ '' ]),
				E('td', { 'class': 'td' }, [
		...
		Object.keys(aclList).sort().forEach(function(aclGroupName) {
			...
			table.appendChild(E('tr', { 'class': 'tr', 'style': (isRequired||isReadable||isWritable) ? '' : 'display: none;' }, [
...
var cbiGroupSelect = form.Value.extend({
	renderWidget: function(section_id) {
		var groupMatches = uci.get('rpcd', section_id, 'groups');
		//if (!groupMatches || !Array.isArray(groupMatches)) { groupMatches = []; }
		if (!groupMatches) { groupMatches=[] } else { groupMatches=groupMatches.toString().split(','); }
		...
		var table = E('table', { 'class': 'table' }, [
			...
			E('tr', { 'class': 'tr' }, [
				E('td', { 'class': 'td' }, [
					...
					E('a', { 'href': '#', 'click': function() {
						table.querySelectorAll('tr:nth-child(n+3)').forEach(function(el) { el.style.display = (el.childNodes[2].childNodes[0].checked) ? '' : 'none'; });
					} }, [ _('selected', 'Selected') ]), ' | ',
					E('a', { 'href': '#', 'click': function() {
						table.querySelectorAll('tr:nth-child(n+3)').forEach(function(el) { el.style.display = ''; });
					} }, [ _('show all', 'All') ])
				...
				E('td', { 'class': 'td' }, [ '' ]),
				E('td', { 'class': 'td' }, [
					_('Member: ', 'Set all group memberships in the table below to one of the given values'),
					E('a', { 'href': '#', 'click': function() {
						table.querySelectorAll('input').forEach(function(el) { el.checked = false; });
					} }, [ _('None') ]), ' | ',
					E('a', { 'href': '#', 'click': function() {
						table.querySelectorAll('input').forEach(function(el) { el.checked = true; });
					} }, [ _('All') ])
				])
			])
		]);

		var ti=0, tsid=uci.resolveSID('rpcd', '@group[' + ti++ + ']'), _name='', _val='';
		while (tsid) {
			_name=uci.get('rpcd', tsid, 'name');
			if (groupMatches.includes(_name)) {
				table.appendChild(E('tr', { 'class': 'tr' }, [
					E('td', { 'class': 'td' }, [ _name ]),
					E('td', { 'class': 'td' }, [ uci.get('rpcd', tsid, 'description') || '-' ]),
					E('td', { 'class': 'td' }, [ E('input', { 'type': 'checkbox', 'enabled': true, 'checked': 'true', 'data-acl-groups': _name }, [ _('member', 'Member of group') ]) ])
				]));
			} else {
				table.appendChild(E('tr', { 'class': 'tr', 'style': 'display: none;' }, [
					E('td', { 'class': 'td' }, [ _name ]),
					E('td', { 'class': 'td' }, [ uci.get('rpcd', tsid, 'description') || '-' ]),
					E('td', { 'class': 'td' }, [ E('input', { 'type': 'checkbox', 'enabled': true, 'data-acl-groups': _name }, [ _('member', 'Member of group') ]) ])
				]));
			}	
			tsid=uci.resolveSID('rpcd', '@group[' + ti++ + ']');
		}
	...
	formvalue: function(section_id) {
		var node = this.map.findElement('data-field', this.cbid(section_id)), data = {};

		node.querySelectorAll('[data-acl-groups]').forEach(function(el) {
			if (el.checked) { data.groups = data.groups || []; data.groups.push(el.getAttribute('data-acl-groups')); }
		});
	...
	write: function(section_id, value) {
		if (L.isObject(value) && Array.isArray(value.groups)) {
			uci.set('rpcd', section_id, 'groups', value.groups.join(','));
		} else {
			uci.set('rpcd', section_id, 'groups', '');
		}
	}
});
...
return view.extend({
	...
	load: function() {
		return L.resolveDefault(fs.list('/usr/share/rpcd/acl.d'), []).then(function(entries) {
			var tasks = [
				L.resolveDefault(fs.stat('/usr/sbin/accessio'), null),
				fs.lines('/etc/passwd')
			];
	...
	render: function(data) {
		...
		m = new form.Map('rpcd', _('Users'), _('The table at hand allows you to configure user accounts, group memberships and permissions in the scope of both LuCI as well as the system passwd files.'));
		...
		o.write = function(section_id, value) {
			var currentvalue=uci.get('rpcd', section_id, 'username');
			if (currentvalue && currentvalue.length) { return; }
			uci.set('rpcd', section_id, 'username', value);
		}

		o = s.option(form.Flag, 'enabled', _('Account Enabled'));
		o.editable = true;
		o.write = function(section_id, formvalue) { uci.set('rpcd', section_id, 'enabled', '1'); };
		o.rmempty = true;
		o.remove = function(section_id) { uci.set('rpcd', section_id, 'enabled', '0'); };


		o = s.option(form.Flag, 'interactive', _('Console Enabled'));
		o.editable = true;
		o.optional = false;
		o.write = function(section_id, formvalue) { uci.set('rpcd', section_id, 'interactive', '1'); };
		o.rmempty = true;
		o.remove = function(section_id) { uci.set('rpcd', section_id, 'interactive', '0'); };
		...
		o.optional = false;
		o.write = function(section_id, value) {
			if (value == '' || value.substring(0, 3) == '$p$') { return; }

			return new Promise((resolve, reject) => {
				var section_name=uci.get('rpcd', section_id, 'username');
				fs.exec('/usr/sbin/accessio', [ 'hash', value ]).then(function(res) {
					if (res.code == 0) {
						uci.set('rpcd', section_id, 'password', '$p$' + section_name);
						var resl = res.stdout.trim().split(/\r\n|\n|\r/);
						for (var i=0; i<resl.length; i++) {
							if (resl[i].startsWith('passwdhash=')) { uci.set('rpcd', section_id, 'passwdhash', resl[i].substring(11)); }
							else if (resl[i].startsWith('smbpasswdhash=')) { uci.set('rpcd', section_id, 'smbpasswdhash', resl[i].substring(14)); }
							else if (resl[i].startsWith('htpasswdhash=')) { uci.set('rpcd', section_id, 'htpasswdhash', resl[i].substring(13)); }
						}
						resolve();
					} else { throw new Error(res.stderr); }
				}).catch(function(err) {
					reject(_('Unable to set user password: %s').format(err.message));
				});
			});
		};
		...
		o = s.option(cbiGroupSelect, '_groups');
		o.modalonly = true;
